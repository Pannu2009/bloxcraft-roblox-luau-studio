...
export const LuadroidKeyBar: React.FC<Props> = ({
  ...
  const tokens = [
    ...
    { label: 'local', insert: 'local ', color: 'text-[#F86D7C] font-bold border-red-500/30 bg-red-950/20' },
    { label: 'function', insert: 'function ', color: 'text-[#F86D7C] font-bold border-red-500/30' },
    ...
    { label: '--!strict', insert: '--!strict\n', color: 'text-[#C678DD] font-semibold border-purple-500/30' },
    { label: 'task.wait()', insert: 'task.wait()', color: 'text-[#84D6F7] font-semibold border-cyan-500/30' },
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 py-1.5 bg-[#121520] border-t border-[#1f2438] overflow-x-auto select-none shrink-0 text-xs font-mono shadow-inner z-10">
      ...
      {onQuickRun && (
        <button
          onClick={onQuickRun}
          className="flex items-center gap-1 px-2.5 py-1 bg-emerald-600/30 hover:bg-emerald-600/40 text-emerald-300 border border-emerald-500/40 rounded-lg text-[11px] font-sans font-bold shrink-0 transition-colors"
      ...
      {onQuickFix && (
        <button
          onClick={onQuickFix}
          disabled={isFixing}
          className="flex items-center gap-1 px-2.5 py-1 bg-red-600 hover:bg-red-500 text-white rounded-lg text-[11px] font-sans font-bold shrink-0 transition-colors shadow-sm disabled:opacity-50"
