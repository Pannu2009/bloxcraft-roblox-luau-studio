      ...
      <div className="bg-[#12141c] border border-[#2b3044] rounded-2xl w-full max-w-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#222738] bg-[#161924]">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-zinc-600/20 border border-zinc-500/30 flex items-center justify-center">
              <span className="font-bold text-zinc-400 text-lg">R</span>
            </div>
            <div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="p-4 rounded-xl bg-gradient-to-r from-zinc-950/30 via-zinc-950/20 to-zinc-950/20 border border-zinc-500/30">
            ...
            <div className="text-sm text-gray-200">
              ...
              {currentScriptType === 'ModuleScript' && (
                <span>
                  Place in <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">ReplicatedStorage</code> (shared between Client & Server) or{' '}
                  <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">ServerScriptService</code> (server-only).
                </span>
              )}
              {currentScriptType === 'ServerScript' && (
                <span>
                  Place in <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">ServerScriptService</code> or inside Workspace models. Never on the client!
                </span>
              )}
              {currentScriptType === 'LocalScript' && (
                <span>
                  Place in <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">StarterPlayer &gt; StarterPlayerScripts</code>,{' '}
                  <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">StarterCharacterScripts</code>, or <code className="text-zinc-300 bg-black/40 px-1.5 py-0.5 rounded font-mono">StarterGui</code>.
                </span>
              )}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-200 mb-3 flex items-center gap-2">
              <Folder className="w-4 h-4 text-zinc-400" /> Roblox Studio Explorer Tree
            </h3>
            ...
            <div className="bg-[#0b0c10] border border-[#212638] rounded-xl p-4 font-mono text-xs space-y-2">
              ...
              {/* ReplicatedStorage */}
              <div className={`pl-4 flex items-center gap-2 py-1 px-2 rounded ${currentScriptType === 'ModuleScript' ? 'bg-zinc-500/10 border border-zinc-500/30 text-zinc-300' : 'text-gray-300'}`}>
              ...
              {/* ServerScriptService */}
              <div className={`pl-4 flex items-center gap-2 py-1 px-2 rounded ${currentScriptType === 'ServerScript' ? 'bg-zinc-500/10 border border-zinc-500/30 text-zinc-300' : 'text-gray-300'}`}>
              ...
              {/* StarterPlayer */}
              <div className={`pl-4 flex items-center gap-2 py-1 px-2 rounded ${currentScriptType === 'LocalScript' ? 'bg-zinc-500/10 border border-zinc-500/30 text-zinc-300' : 'text-gray-300'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="p-3 bg-[#171a26] border border-[#252b3d] rounded-xl">
              <div className="text-xs font-bold text-zinc-400 mb-1">Step 1: Copy Code</div>
              <p className="text-xs text-gray-300">
                Click the <strong className="text-white">"Copy Script"</strong> button in BloxCraft AI.
              </p>
            </div>
            <div className="p-3 bg-[#171a26] border border-[#252b3d] rounded-xl">
              <div className="text-xs font-bold text-zinc-400 mb-1">Step 2: Create in Studio</div>
              <p className="text-xs text-gray-300">
            ...
            <div className="p-3 bg-[#171a26] border border-[#252b3d] rounded-xl">
              <div className="text-xs font-bold text-zinc-400 mb-1">Step 3: Paste & Rename</div>
              <p className="text-xs text-gray-300">
                Paste the code with <kbd className="bg-black/50 px-1 py-0.5 rounded text-white font-mono">Ctrl+V</kbd> and rename to <code className="text-zinc-300 font-mono">{currentScriptName.replace('.luau', '')}</code>.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-[#222738] bg-[#161924] flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 bg-zinc-600 hover:bg-zinc-500 text-white font-medium text-sm rounded-xl transition-all shadow-lg shadow-zinc-900/30"
          >
            Got it, thanks!
          </button>
        </div>
